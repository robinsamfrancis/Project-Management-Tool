...
function BoardPage() {
  ...
  const allItems = useMemo(() => {
    let result = store.items.filter((i) => i.workspaceCode === code);
    if (isProjectMember) {
      result = result.filter((i) => i.assigneeId === myUserId);
    }
    return result;
  ...
  const filteredItems = useMemo(() => {
    ...
    let itemsResult = allItems.filter((i) => {
      // Features should not appear in active sprint boards
        ...
        {workspace.statuses.map((status) => {
          ...
          return (
            <div
              ...
              onDragOver={
                !isProjectMember ? (e) => e.preventDefault() : undefined
              }
              onDrop={
                !isProjectMember
                  ? () => {
...
function BoardCard({
  ...
  return (
    <div
      ...
      onDragStart={!isProjectMember ? onDragStart : () => {}}
      onDragEnd={!isProjectMember ? onDragEnd : () => {}}
