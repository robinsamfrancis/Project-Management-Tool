...
function DashboardPage() {
  ...
  const isOwner =
    isPortfolioManager ||
    userRole === "Admin User" ||
    (workspace?.ownerIds.includes(myUserId) ?? false);
  const isMember = workspace?.memberIds.includes(myUserId) || false;
  const canSwitchView =
    isOwner && isMember && !isPortfolioManager && userRole !== "Admin User";

  const [viewModePref, setViewModePref] = useState<"team" | "my">("team");
  const viewMode = canSwitchView ? viewModePref : isOwner ? "team" : "my";
  const isProjectMember =
    viewMode === "my" && !isPortfolioManager && userRole !== "Admin User";

  const rawItems = useMemo(() => {
    let items = store.items.filter(
      (i) => i.workspaceCode === code && i.type !== "feature",
    );
    if (isProjectMember) {
      items = items.filter((i) => i.assigneeId === myUserId);
    }
    return items;
  }, [store.items, code, isProjectMember, myUserId]);
  ...
  const sprintFilteredRawItems = useMemo(() => {
    return rawItems.filter((item) => {
      if (sprintFilter === "active") {
  ...
  }, [rawItems, sprintFilter, activeSprint, sprints]);
  ...
  // -------------------------------------------------------------
  // calculations: TEAM WORKLOAD GRID -> PERSONAL CAPACITY
  // -------------------------------------------------------------
  const teamWorkload = useMemo(() => {
    const listToProcess = isProjectMember
      ? [DIRECTORY.find((p) => p.id === myUserId)]
      : workspaceMembers;
    ...
    return listToProcess
      .map((person) => {
        ...
        const leaveHours = 0; // Mock leave hours
        const availabilityHours = 80 - leaveHours;
        const utilization = Math.round(
          (allocatedHours / availabilityHours) * 100,
        );

        let statusColor: "success" | "warning" | "destructive" = "success";
        let statusText = "Optimally Utilized";
  ...
  }, [filteredItems, isProjectMember, workspaceMembers, myUserId]);
  ...
  const velocityChartData = useMemo(() => {
    ...
    const myItems = rawItems.filter((i) => i.assigneeId === myUserId);

    const data = historicalSprints.map((s) => {
      // Completed points for each sprint
      const sprintCompletedPoints = myItems
        .filter((i) => i.sprintId === s.id && i.status === "Completed")
        .reduce((sum, i) => sum + (i.points || 0), 0);

      const sprintPlannedPoints = myItems
        .filter((i) => i.sprintId === s.id)
        .reduce((sum, i) => sum + (i.points || 0), 0);

      // Force personal scale down by avoiding fallback object property like s.completedPoints
      return {
        name: s.name,
        Completed:
          sprintCompletedPoints || Math.round(sprintPlannedPoints * 0.85) || 8,
        Planned: sprintPlannedPoints || 10,
      };
    });
    ...
    if (data.length === 0) {
      return [
        { name: "Sprint 7", Completed: 8, Planned: 10 },
        { name: "Sprint 8", Completed: 10, Planned: 12 },
        { name: "Sprint 9", Completed: 9, Planned: 10 },
        { name: "Sprint 10 (Current)", Completed: 12, Planned: 14 },
      ];
    }

    return data;
  }, [sprints, rawItems, myUserId]);
  ...
  const dependencyRisks = useMemo(() => {
    return sprintFilteredRawItems
      .flatMap((i) => i.dependencyRisks || [])
      .filter((dr) => dr.ownerIds?.includes(myUserId));
  }, [sprintFilteredRawItems, myUserId]);
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-4">
            ...
            {canSwitchView && (
              <Select
                value={viewModePref}
                onValueChange={(v) => setViewModePref(v as "team" | "my")}
              >
                <SelectTrigger className="h-8 w-[160px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="team">Team Dashboard</SelectItem>
                  <SelectItem value="my">My Dashboard</SelectItem>
                </SelectContent>
              </Select>
            )}
          </div>
          <p className="text-xs text-muted-foreground">
            {viewMode === "team"
              ? "Monitor real-time velocity, burden rate, team allocation, and item cycles."
              : "Monitor your assigned tasks, metrics, and sprint progress."}
          </p>
        </div>
          ...
          <CardContent className="flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div className="max-w-md space-y-1">
              ...
              <CardDescription className="text-xs text-muted-foreground">
                You have no items assigned to you in the selected sprint. Work
                with your manager to pull in stories or tasks.
              </CardDescription>
            </div>
          </CardContent>
        </Card>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="hover:shadow-md transition-shadow relative overflow-hidden bg-gradient-to-br from-card to-card/70 border border-border">
              <div className="absolute top-0 right-0 h-20 w-20 translate-x-4 -translate-y-4 rounded-full bg-info/5 blur-xl" />
              ...
              <CardContent>
                <div className="text-xs text-muted-foreground">
                  Active workspaces tasks requiring your action
                </div>
              </CardContent>
            </Card>

            {/* KPI 1: Completed Work */}
            <Card className="hover:shadow-md transition-shadow relative overflow-hidden bg-gradient-to-br from-card to-card/70 border border-border">
              ...
              <CardContent>
                <div className="flex flex-wrap items-center gap-2">
                  {dueStats.overdue > 0 ? (
                    <Badge
                      variant="destructive"
                      className="h-auto py-1 px-2.5 inline-flex items-center gap-1 font-bold animate-pulse whitespace-nowrap leading-snug"
                    >
                      <AlertCircle className="h-3 w-3 shrink-0" />{" "}
                      {dueStats.overdue} Overdue
                    </Badge>
                  ) : (
                    <span className="text-success-600 bg-success-50 dark:bg-success-950/25 h-auto py-1 px-2.5 inline-flex items-center rounded font-semibold text-[11px] whitespace-nowrap leading-snug">
                      0 Overdue
                    </span>
                  )}
            ...
            <Card className="hover:shadow-md transition-shadow relative overflow-hidden bg-gradient-to-br from-card to-card/70 border border-border">
              ...
              <CardContent>
                <div className="flex flex-wrap items-center gap-2">
                  <Badge
                    variant="outline"
                    className="h-auto py-1 px-2.5 inline-flex items-center gap-1 bg-primary/10 border-primary/20 text-primary font-bold whitespace-nowrap leading-snug"
                  >
                    {updatedStats.todayCount} Today
                  </Badge>
                  <span className="text-muted-foreground text-xs leading-snug">
                    active record updates detected
                  </span>
                </div>
              </CardContent>
            </Card>
          ...
          <Card className="border border-border">
            <CardHeader className="pb-3 border-b border-border/40">
              <CardTitle className="text-sm font-semibold text-foreground flex items-center gap-2">
                <Users2 className="h-4 w-4 text-indigo-500" /> Personal
                Occupancy
              </CardTitle>
              <CardDescription className="text-xs">
                Audits your real-time load distribution based on a standard
                80-hour sprint capacity minus leave duration.
              </CardDescription>
            </CardHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                {filteredItems.length === 0 ? (
                  <div className="flex h-full flex-col items-center justify-center text-center space-y-2 opacity-60">
                    ...
                    <p className="text-xs text-muted-foreground w-3/4">
                      No Burndown Data Available. Wait for tasks to be assigned
                      to you.
                    </p>
                  </div>
                ) : (
            ...
            <Card className="lg:col-span-2 border border-border flex flex-col justify-between">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-semibold text-foreground flex items-center justify-between">
                  Personal Sprint Burndown Widget
                  <Award className="h-4 w-4 text-emerald-500" />
                </CardTitle>
                <CardDescription className="text-xs">
                  Your performance velocity forecast for the current active
                  sprint.
                </CardDescription>
              </CardHeader>
            ...
            <CardHeader className="pb-3 border-b border-border/40 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="space-y-0.5">
                <CardTitle className="text-sm font-semibold text-foreground flex items-center gap-1.5 animate-pulse">
                  <TrendingUp className="h-4 w-4 text-emerald-500" /> Personal
