...
function BacklogPage() {
  ...
  const allItems = useMemo(() => {
    let filtered = store.items.filter((i) => i.workspaceCode === code);
    if (isProjectMember) {
      filtered = filtered.filter((i) => i.assigneeId === myUserId);
    }
    return filtered;
  ...
  const matches = useMemo(() => {
    return (i: BacklogItem) => {
      // Features should not appear in Backlog lists
    ...
    <div className="mx-auto w-full max-w-7xl px-4 lg:px-8 py-6 space-y-6">
      ...
      {noWorkspaceContent && (
        <EmptyState
          ...
          description={
            isProjectMember
              ? "There are no items attached to this workspace assigned to you."
              : "Start by creating a sprint and adding backlog items. Drag items into a sprint to plan work."
          }
          actionLabel={isProjectMember ? undefined : "Create your first sprint"}
          onAction={
            isProjectMember
              ? undefined
              : () => {
        ...
        <div className="space-y-4">
          ...
          {activeSprints.map((sp) => (
            <SprintCard
              ...
              onAddItem={!isProjectMember ? () => openCreate(sp.id) : undefined}
              onComplete={
                !isProjectMember
                  ? () => {
          ...
          {upcomingSprints.map((sp) => (
            <SprintCard
              ...
              onDelete={!isProjectMember ? (id) => deleteItem(id) : () => {}}
              onAddItem={!isProjectMember ? () => openCreate(sp.id) : undefined}
              onStart={
                !isProjectMember
                  ? () => {
          ...
          <BacklogSection
            ...
            onAddItem={!isProjectMember ? () => openCreate() : undefined}
            isProjectMember={isProjectMember}
          />
        </div>
      )}
      ...
      <ItemFormDialog
        ...
        onCreated={(item) => {
          if (item.type === "feature") {
            navigate({ to: `/workspaces/${code}/roadmap` });
          }
        }}
      />
      <SprintFormDialog
          ...
          {items.length === 0 ? (
            ...
            items.map((i) => (
              <IssueRow
                ...
                onDragStart={
                  !isProjectMember ? () => setDragId(i.id) : () => {}
                }
                onDragEnd={!isProjectMember ? () => setDragId(null) : () => {}}
                onEdit={!isProjectMember ? () => onEdit(i) : undefined}
                onDelete={!isProjectMember ? () => onDelete(i.id) : undefined}
                isProjectMember={isProjectMember}
              />
            ))
          )}
...
function BacklogSection({
  ...
  const quickCreate = () => {
    if (!quick.trim() || isProjectMember) return;
    createItem({
      workspaceCode: code,
      type: "task",
      title: quick.trim(),
      ...
      {items.length === 0 ? (
        <div className="p-8">
          <EmptyState
            ...
            actionLabel={onAddItem ? "Create your first item" : undefined}
            onAction={onAddItem}
        ...
        <div className="divide-y divide-border">
          {items.map((i) => (
            <IssueRow
              ...
              statuses={workspaceStatuses}
              isDragging={dragId === i.id}
              onDragStart={!isProjectMember ? () => setDragId(i.id) : () => {}}
              onDragEnd={!isProjectMember ? () => setDragId(null) : () => {}}
              onEdit={!isProjectMember ? () => onEdit(i) : undefined}
              onDelete={!isProjectMember ? () => onDelete(i.id) : undefined}
              isProjectMember={isProjectMember}
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex items-center gap-2">
        ...
        <InlineEpicFeaturePicker item={item} disabled={isProjectMember} />
      </div>
      <InlineDatePicker item={item} disabled={isProjectMember} />
      <Badge
        variant="outline"
